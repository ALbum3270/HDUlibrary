...
class ConfigManager:
    ...
    def get_user_info(self) -> Dict[str, str]:
        """Get user credentials, with environment variables taking priority.

        Environment overrides keep credentials out of repository files when the
        application runs in CI.  The SEATHUNTER_* names are preferred; the old
        SCHOOL_ID/PASSWORD names are supported for compatibility with the
        original GitHub Actions project.
        """
        user = dict(self.config.get("user", {}))
        login_name = (
            os.environ.get("SEATHUNTER_LOGIN_NAME")
            or os.environ.get("SCHOOL_ID")
        )
        password = (
            os.environ.get("SEATHUNTER_PASSWORD")
