...
class Schedule:
    ...
    def plan_ids_for_date(self, target_date: datetime) -> List[str]:
        """Return the plans this schedule assigns to a target calendar date."""
        if not self.enabled:
            return []

        if self.mode == "weekdays":
            if target_date.isoweekday() in self.target_weekdays:
                return list(self.plan_ids)
            return []

        target_date_str = target_date.strftime("%Y-%m-%d")
        plan_ids = []
        for mapping in self.mappings:
            if mapping.target_date == target_date_str:
                plan_ids.extend(mapping.plan_ids)
        return plan_ids
